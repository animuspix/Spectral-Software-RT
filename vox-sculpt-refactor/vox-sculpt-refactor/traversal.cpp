...
void traversal::iterate(tracing::path_vt init_vt, tracing::path* vertex_output, float* isosurf_dist, uint32_t tileNdx)
{
...
//#define VALIDATE_BOUNDARY_SCATTERING
    ...
    while (!path_absorbed && !path_escaped)
    {
        ...
        if (within_grid) // Within the grid, march until we leave the bounding box
        {
            ...
            vmath::vec<3> uvw_scaled = uvw * vol_constants::width; // Voxel coordinates! :D
            vmath::vec<3, int32_t> uvw_i = vmath::vec3_cast<vmath::vec<3>, vmath::vec<3, int32_t>>(vmath::vfloor(vmath::vabs(uvw_scaled))); // Probably paranoid, but voxel coordinates should never be negative
...
//#define VALIDATE_STEPPED_RO
            ...
            {
                uvw_i = vmath::vmax(uvw_i, vmath::vec<3, int32_t>(0, 0, 0));
                within_grid = false;
            }
            /*else
            {
            ...
            if (within_grid)
            {
                ...
                case material_labels::DIFFUSE:
                    float sample[4];
                    parallel::GetRNGStream(tileNdx).next(sample);
