...
namespace vmath
{
    ...
    // Quaternions! 4D (concatenations) and 3D (application)
    // Need to refresh my memory of how this math works...
    template<vec4_type vec4>
    vec4 qtn_rotation_concat(vec4 p, vec4 q)
    {
    ...
    template<vec4_type vec4, vec3_type vec3>
    vec3 qtn_rotation_apply(vec3 v, vec4 q)
    {
        //platform::osAssertion(q.sqr_magnitude() == 1.0f); // Rotation quaternions should have unit magnitude
        vec<4> p = vec<4>(v.x(), v.y(), v.z(), 0.0);
        vec<4> qv = qtn_rotation_concat(p, q);
        vec<4> q_inv = vec<4>(qv.x() * -1.0f,
                              qv.y() * -1.0f,
                              qv.z() * -1.0f, q.w());
        return qtn_rotation_concat(qv, q_inv).xyz();
    }
