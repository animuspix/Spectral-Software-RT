...
#define DEMO_SPECTRAL_PT
				...
				float distX = ((float)vmath::abs(vmath::abs(x - ui::image_centre_x) - ui::image_centre_x)) / (float)ui::image_centre_x;
				float distY = ((float)vmath::abs(vmath::abs(y - ui::image_centre_y) - ui::image_centre_y)) / (float)ui::image_centre_y;
				float dist = 1.0f - sqrt(distX * distX + distY * distY);

				float sample[4];
				auto camVt = camera::lens_sample(x, y, sample[0], sample[1], dist);
				camera::sensor_response(dist, 1.0f, 1.0f, 1.0f, pixel_ndx, sample_ctr[tileNdx]);
				camera::tonemap_out(pixel_ndx);
#elif defined(DEMO_AND)
				float sample[4];
				float rho = float(x & y) / (float)y;
				auto camVt = camera::lens_sample(x, y, sample[0], sample[1], rho);

				camera::sensor_response(rho, 1.0f, 1.0f, 1.0f, pixel_ndx, sample_ctr[tileNdx]);
				camera::tonemap_out(pixel_ndx);
#elif defined(DEMO_XOR)
				float sample[4];
				float rho = float((x ^ y) % ui::window_area) / float(x * y / 8);
				auto camVt = camera::lens_sample(x, y, sample[0], sample[1], rho);
				...
				auto camVt = camera::lens_sample(x, y, sample[0], sample[1], sample[0]);

				camera::sensor_response(sample[0], 1.0f, 1.0f, 1.0f, pixel_ndx, sample_ctr[tileNdx]);
				camera::tonemap_out(pixel_ndx);
#elif defined (DEMO_FILM_RESPONSE) // Rainbow gradient test
				float sample[4];
				parallel::rand_streams[tileNdx].next(sample);

				auto camVt = camera::lens_sample(x, y, sample[0], sample[1], float(x) / float(ui::window_width));
				camera::sensor_response(camVt.rho_sample, 1.0f, 1.0f, 1.0f, pixel_ndx, sample_ctr[tileNdx]);
				camera::tonemap_out(pixel_ndx);
