...
namespace geometry
{
	export struct vol
	{
			...
			// [uvw_floored] is expected in voxel space (0...vol::width on each axis)
		{
			...
			u8 bit_ndx = static_cast<u8>((floor(std::fmodf(uvw_floored.z(), 4.0f)) * 4) + // Z-axis, snapped into chunk space
				(floor(std::fmodf(uvw_floored.y(), 4.0f)) * 16) + // Y-axis, snapped into chunk space
				floor(std::fmodf(uvw_floored.x(), 4.0f))); // X-axis, snapped into chunk space
